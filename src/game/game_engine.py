# FULLY DEBUGGED Game Engine - All Issues Fixed
"""
src/game/game_engine_debug_fixed.py
Completely debugged and fixed game engine with comprehensive logging
"""
...
class GameEngine:
    def __init__(self, mode="human"):
        """Initialize game engine with debugging"""
        pygame.init()
        pygame.mixer.init()
        ...
        # Game objects
        self.birds = []
        self.pipe_manager = PipeManager(self.asset_loader.get_pipe_sprite("GREEN"))
    ...
    def init_ai_training(self):
        ...
        if self.genetic_algorithm is None:
            print(f"🧬 Initializing Genetic Algorithm with population size: {self.population_size}")
    ...
    def create_ai_birds_with_debugging(self):
        ...
        if not hasattr(self.genetic_algorithm, 'population') or not self.genetic_algorithm.population.individuals:
            ...
            for i, bird in enumerate(self.birds):
                ...
                brain = NeuralNetwork(NN_INPUT_NODES, NN_HIDDEN_NODES, NN_OUTPUT_NODES, seed=seed)
                bird.brain = brain
                
                # DEBUG: Log neural network weights for first few birds
                if i < 5:
                    weights = brain.get_weights_as_array()
                    print(f"   Bird {i} brain: {len(weights)} params, sample weights: {weights[:5]}")
        else:
            ...
            if len(self.birds) >= 2:
                ...
                print(f"🧬 Brain diversity check: avg difference = {difference:.4f}")
                if difference < 0.001:
        ...
        print(f"✅ Created {len(self.birds)} AI birds with diverse neural networks")
    ...
    def test_neural_network_diversity(self):
        ...
        for i, test_input in enumerate(test_inputs):
            ...
            print(f"   Test {i+1}: {unique_decisions}/{len(decisions)} unique decisions")
    ...
    def update_game(self):
        """Update game logic with debugging"""
    ...
    def update_birds_with_debugging(self):
        ...
        for i, bird in enumerate(self.birds):
            if bird.alive:
                ...
                if self.mode == "human":
                    ...
                    if bird.brain:
                        ...
                        if i == 0 and self.generation_frame_count % 10 == 0:
                            print(f"Frame {self.generation_frame_count}: Bird state = {[f'{x:.3f}' for x in game_state]}")
                        ...
                        try:
                            ...
                            if i < 3 and self.generation_frame_count % 15 == 0:
                                print(f"   Bird {i}: NN output={output:.3f}, jump={jump}")
                ...
                collision = self.check_bird_collision_detailed(bird, ground_y, i)
                
                if collision:
                    # DEBUG: Log collision details
                    if self.debug_mode and i < 3:
                        print(f"   Bird {i} collision at frame {self.generation_frame_count}: y={bird.rect.y}, reason={collision}")
        ...
        if self.generation_frame_count % 30 == 0 and alive_count > 0:
            ...
            if total_decisions > 0:
                ...
                print(f"Frame {self.generation_frame_count}: {alive_count} alive, {jump_pct:.1f}% jumping")
            ...
            for pipe in pipes:
                if bird.rect.colliderect(pipe.rect):
                    ...
                    try:
                        ...
                        offset = (pipe.rect.x - bird.rect.x, pipe.rect.y - bird.rect.y)
        ...
        if self.mode == "ai_training":
            ...
            if should_end:
                if generation_time > 60:
                    ...
                    print(f"⏰ Generation frame limit reached: {self.generation_frame_count}")
    ...
    def end_generation(self):
        ...
        for i, bird in enumerate(self.birds):
            ...
            survival_time = self.generation_frame_count if bird.alive else bird.fitness / FITNESS_BONUS_DISTANCE
        ...
        best_score = max(bird.score for bird in self.birds) if self.birds else 0
        max_survival = max(bird.fitness / FITNESS_BONUS_DISTANCE for bird in self.birds) if self.birds else 0
        ...
        # Show top performers
        top_performers = sorted(individual_stats, key=lambda x: x["fitness"], reverse=True)[:3]
        print(f"   🏅 Top performers:")
        for i, perf in enumerate(top_performers):
            print(f"      {i+1}. Bird {perf['id']}: fitness={perf['fitness']:.1f}, score={perf['score']}, survival={perf['survival']:.1f}")
    ...
    def render_game(self):
        ...
        # Draw background
        self.renderer.draw_background("DAY")
    ...
    def draw_ai_training_ui(self):
        ...
        if self.birds:
            ...
            avg_fitness = sum(current_fitness) / len(current_fitness) if current_fitness else 0
        else:
            best_fitness = 0
            avg_fitness = 0
        
        # Display comprehensive AI training info
        self.renderer.draw_ai_info(self.generation, alive_birds, best_fitness, avg_fitness, len(self.birds))
